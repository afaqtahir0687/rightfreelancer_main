...
namespace plugins\FormBuilder;

use Illuminate\Support\Str;
use plugins\FormBuilder;
...
HTML;
    ...
    public static function render_drag_drop_form_builder($content = '')
    {
        ...
        if (! empty($form_fields)) {
            ...
            foreach ($form_fields->field_type as $key => $ftype) {
               ...
               if ($ftype === 'select') {
                    $field_name = $form_fields->field_name[$key] ?? null;
                    if ($field_name && isset($form_fields->select_options->$field_name)) {
                        $args['select_option'] = implode("\n", $form_fields->select_options->$field_name);
                    } else {
                        $args['select_option'] = '';
                    }
                }
    ...
    private static function form_builder_field_markup($key, $type, $name, $placeholder, $required, $args = [])
    {
        ...
        $output = '<li class="ui-state-default mt-4">
                     <span class="ui-icon ui-icon-arrowthick-2-n-s"></span>
                    <span class="remove-fields me-2"><i class="fa-solid fa-times"></i></span>
                    <a data-bs-toggle="collapse" href="#fileds_collapse_'.$key.'" role="button"
                       aria-expanded="false" aria-controls="collapseExample" class="w-100">
                        '.ucfirst($type).': <span
        ...
        $output .= '<div class="collapse" id="fileds_collapse_'.$key.'">
            <div class="card card-body mt-3">
                ...
                <div class="form-group single-input mt-3">
                    <label class="label-title">'.__('Placeholder/Label').'</label>
                    <input type="text" class="form-control field-placeholder"
                           name="field_placeholder[]" placeholder="'.__('enter field placeholder/label').'"
                           value="'.$placeholder.'" >
                </div>
                <div class="form-group single-input mt-3">
                    <label for="required"><strong>'.__('Required').'</strong></label>
                    <label class="switch">
                        <input type="checkbox" id="required" class="field-required" '.$required_check.' name="field_required['.$key.']">
