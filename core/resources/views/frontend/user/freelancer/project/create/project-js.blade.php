    ...
    (function ($) {
        ...
        $(document).ready(function () {
            ...
            document.getElementById('upload_project_photo').addEventListener('change', function () {
                ...
                Array.from(files).forEach(file => {
                    ...
                    if (fileType.startsWith('image/')) {
                        ...
                        // Show file icon for document types
                        const iconPath = `/assets/img/file-icons/${ext}.png`; // fallback if available
            ...
            $(document).on('click', '#confirm_create_project', async function(e) {
                ...
                if (!basic_details) missingPackages.push("Basic");
                else if (basic_details.length > 150) limitExceeded.push("Basic");

                if (!standard_details) missingPackages.push("Standard");
                else if (standard_details.length > 150) limitExceeded.push("Standard");

                if (!premium_details) missingPackages.push("Premium");
                else if (premium_details.length > 150) limitExceeded.push("Premium");

                if (missingPackages.length > 0) {
                    let message = "Please fill " + missingPackages.join(", ") + " package" + (missingPackages.length > 1 ? "s" : "") + " details.";
                ...
                // Check for duplicate or empty checkbox/numeric fields
