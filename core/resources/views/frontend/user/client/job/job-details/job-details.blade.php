...
@section('content')
    <main>
        ...
        <div class="profile-area pat-100 pab-100 section-bg-2">
                                    ...
                                    <div class="myJob-wrapper-single-flex flex-between align-items-center">
                                        <div class="myJob-wrapper-single-contents">
                                            <div class="flex-btn">
                                                ...
                                                <!-- Share Job Button -->
                                                <button type="button" class="btn-profile btn-bg-1 btn-sm ms-2" data-bs-toggle="modal" data-bs-target="#shareJobModal">
                                                    <i class="fas fa-share-alt me-1"></i> {{ __('Share Job') }}
                                                </button>
                                            </div>
                                            ...
                                            <li class="jobFilter-wrapper-item-bottom-list-item">
                                                <span class="item-icon">
                                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none"
                                                        ...
                                                        <path
                                                                d="M14.6836 8.0166H10.3086C9.96693 8.0166 9.68359 7.73327 9.68359 7.3916C9.68359 7.04993 9.96693 6.7666 10.3086 6.7666Z"
                                                                fill="#475467" />
                                                        <path
            ...
            @php
                $jobUrl = '';
                if ($job_details->job_creator && $job_details->slug) {
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content radius-10">
                        ...
                        <div class="modal-body text-center">
                            <!-- Job Preview (attachment image/icon if available) -->
                            <!-- @php
                                $jobAttachment = $job_details->attachment ?? null;
                                $jobAttachmentUrl = null;
                                $jobAttachmentExt = $jobAttachment ? strtolower(pathinfo($jobAttachment, PATHINFO_EXTENSION)) : null;
                                $dummyJobIcon = 'https://ssl.gstatic.com/docs/doclist/images/mediatype/icon_1_document_x16.png';
                                if ($jobAttachment) {
                                    $jobAttachmentUrl = (cloudStorageExist() && in_array(Storage::getDefaultDriver(), ['s3','cloudFlareR2','wasabi']))
                                        ? render_frontend_cloud_image_if_module_exists('job/'.$jobAttachment, load_from: $job_details->load_from)
                                        : asset('assets/uploads/jobs/'.$jobAttachment);
                                }
                            @endphp
                            <div class="mb-3" id="shareJobMainPreview">
                                @if($jobAttachmentUrl && in_array($jobAttachmentExt, ['jpg','jpeg','png','bmp','tiff','svg','webp']))
                                    <img src="{{ $jobAttachmentUrl }}" alt="{{ __('Job Attachment') }}" class="rounded mb-2" width="150px">
                                @elseif($jobAttachmentUrl && $jobAttachmentExt === 'pdf')
                                    <a href="{{ $jobAttachmentUrl }}" target="_blank">
                                        <img src="https://ssl.gstatic.com/docs/doclist/images/mediatype/icon_1_pdf_x16.png" alt="PDF" class="mb-2" width="40px">
                                        <div class="small text-muted">{{ basename($jobAttachment) }}</div>
                                    </a>
                                @elseif($jobAttachmentUrl)
                                    <a href="{{ $jobAttachmentUrl }}" target="_blank" download>
                                        <img src="https://ssl.gstatic.com/docs/doclist/images/mediatype/icon_1_document_x16.png" alt="Attachment" class="mb-2" width="40px">
                                        <div class="small text-muted">{{ basename($jobAttachment) }}</div>
                                    </a>
                                @else
                                    <img src="https://ssl.gstatic.com/docs/doclist/images/mediatype/icon_1_document_x16.png" alt="{{ __('Job') }}" class="rounded mb-2" width="60px">
                                @endif
                            </div> -->
                            <h5>{{ $job_details->title }}</h5>
                            <div class="text-muted" style="max-height:120px; overflow:auto; text-align:left; margin-bottom:10px; text-align: center;">
                                {{ Str::limit(strip_tags($job_details->description), 120) }}
                            </div>
                            @if($jobUrl)
                                <div class="d-flex justify-content-center gap-3 mt-4">
                                    <!-- WhatsApp -->
                                    <a href="https://wa.me/?text={{ urlencode('Check out this job: ' . $jobUrl) }}"
                                       target="_blank" class="btn btn-success btn-icon" title="WhatsApp">
                                        <i class="fab fa-whatsapp"></i>
                                    </a>
                                    <!-- Twitter -->
                                    <a href="https://twitter.com/intent/tweet?url={{ urlencode($jobUrl) }}"
                                       target="_blank" class="btn btn-primary btn-icon" title="Twitter" style="background-color: #1DA1F2; border-color: #1DA1F2;">
                                        <i class="fab fa-twitter"></i>
                                    </a>
                                    <!-- LinkedIn -->
                                    <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode($jobUrl) }}"
                                    ...
                                    <!-- Copy Link -->
                                    <a href="javascript:void(0);" class="btn btn-dark btn-icon copy-job-link" title="Copy Link" data-link="{{ $jobUrl }}">
                                        <i class="fas fa-link"></i>
                                    </a>
                                </div>
                            @else
                                <div class="alert alert-warning mt-4 text-center">
                                    {{ __('The job link is currently unavailable. Please check the job details or contact support if you believe this is an error.') }}
                                </div>
                            @endif
                        </div>
                    </div>
...
@section('script')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            document.querySelectorAll('.copy-job-link').forEach(function (btn) {
